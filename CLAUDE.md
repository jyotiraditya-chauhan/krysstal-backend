...
- Current rule for `admins`: a signed-in user may read their own `admins/{uid}` doc, and any signed-in user whose token carries `role: 'Admin'` may read any doc in the collection (that's what lets the admin console list all staff directly, with no `listUsers` function). `create`/`delete` are fully denied — only `users.service.ts` (Admin SDK) can create a doc. `update` has one narrow, field-scoped exception: a user may update *only* `mustChangePassword`/`updatedAt` on their own doc, so the frontend can clear the flag after a forced first-login password change — nothing else is writable from a client. Never widen that `hasOnly([...])` list — `role`/`status` must stay Admin-SDK-only, or a client could self-promote their own role.
